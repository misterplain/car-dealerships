...
try {
  ...
  // console.log(
  //   `Loaded ${dealerships_data.dealerships?.length || 0} dealerships`
  // );
...
app.get("/fetchReviews/dealer/:id", (req, res) => {
  try {
    const dealerReviews = reviews.filter(
      (review) => review.dealership == req.params.id
    );
...
app.get("/fetchDealers/:state", (req, res) => {
  try {
    const stateDealers = dealerships.filter(
      (dealer) => dealer.state === req.params.state
    );
    console.log(
      `Returning ${stateDealers.length} dealerships for state ${req.params.state}`
    );
